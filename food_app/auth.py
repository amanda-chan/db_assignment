...
def register():
    if request.method == 'POST':
        ...
        if user:
            ...
            new_user = Customer(email=email, username=username, password=generate_password_hash(
                password, method='sha256'), contact_number=contact_number)
            sql_db.session.add(new_user)
            sql_db.session.commit()
            login_user(new_user, remember=True)
            flash('Account created!', category='success')
            return redirect(url_for('views.home')) # todo: redirect to home
